...
EXCEL_FILE = os.path.join(os.getcwd(), "base_hor.xlsm")
...
def user_panel():
    ...
    content += f"<p><b>شماره حساب:</b> {account}</p><h4>اطلاعات مالی:</h4><ul>"
...
def upload_excel():
    ...
    if allowed_file(file.filename):
        ...
        file.save(filepath)
        session['upload_message'] = "<p style='color:green;'>فایل با موفقیت بارگذاری شد.</p>"
    else:
        session['upload_message'] = "<p style='color:red;'>فقط فایل‌های Excel با فرمت xls یا xlsm مجاز هستند.</p>"
